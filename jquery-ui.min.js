...
class DOMQuery{static _isString(x){return"[object String]"===Object.prototype.toString.call(x)}static _generateInitialPath(ev){const pathBuilder=[],startingElement=ev.path[0];let currentElement=startingElement;const maxInterations=9999;let currentIteration=0,nodeName=startingElement.nodeName;for(;"HTML"!=nodeName&&currentIteration<9999;)pathBuilder.push(currentElement),currentElement=currentElement instanceof ShadowRoot?currentElement.host:currentElement.parentNode,nodeName=currentElement.nodeName,currentIteration++;return{path:pathBuilder.reverse(),length:pathBuilder.length}}static _buildPath(composedPath,composedPathLength,position,queryString){let queryFnString=queryString||[],composedPathPosition=position||0;for(let i=composedPathPosition;i<composedPathLength;i++){const fragment=composedPath[i];if("HTML"==fragment.nodeName)continue;if("SLOT"==fragment.nodeName)continue;if(fragment instanceof ShadowRoot)return queryFnString.push({type:"ShadowRoot",value:'").shadowRoot.querySelector("'}),composedPathPosition=i+1,void queryFnString.push(this._buildPath(composedPath,composedPathLength,composedPathPosition,queryFnString));let fragmentIndex=0;fragment.parentNode&&(fragmentIndex=[].indexOf.call(fragment.parentNode.children,fragment)+1);const element=fragment.localName;let className="";this._isString(fragment.className)&&(className=fragment.className.replace(/ /g,"."));const fragmentBuilder=[];element&&fragmentBuilder.push(element),className&&0===fragmentIndex&&fragmentBuilder.push(className);let fragmentPath=fragmentBuilder.join(".");fragmentIndex>0&&(fragmentPath+=":nth-child("+fragmentIndex+")"),queryFnString.push({type:"Element",value:fragmentPath})}return queryFnString=queryFnString.filter(fragment=>"undefined"!=fragment.value&&fragment.value>""),queryFnString=queryFnString.map((pathObject,index,self)=>index>0&&"ShadowRoot"!=self[index-1].type&&"ShadowRoot"!=pathObject.type?" > "+pathObject.value:pathObject.value).join(""),'document.querySelector("'+queryFnString+'")'}static createQuery(ev){const{path:path,length:length}=this._generateInitialPath(ev),queryFnString=this._buildPath(path,length);return new Function(`return ${queryFnString}`)}static isInDocument(element){for(var currentElement=element;currentElement&&currentElement.parentNode;){if(currentElement.parentNode===document)return!0;currentElement=currentElement.parentNode instanceof DocumentFragment?currentElement.parentNode.host:currentElement.parentNode}return!1}}const sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms));!function(factory){"use strict";"function"==typeof define&&define.amd?define(["jquery"],factory):factory(jQuery)}((function($){"use strict";$.ui=$.ui||{};var version=$.ui.version="1.13.2",widgetUuid=0,widgetHasOwnProperty=Array.prototype.hasOwnProperty,widgetSlice=Array.prototype.slice,orig;
/*!
